...
$(document)
		.ready(
				function() {
					...
					if ($.fn.wl_Autocomplete)
						$.fn.wl_Autocomplete.defaults = {
						// check http://jqueryui.com/demos/autocomplete/ for all
						// options
						};

					if ($.fn.wl_Breadcrump)
						$.fn.wl_Breadcrump.defaults = {
							start : 0,
							numbers : false,
							allownextonly : false,
							disabled : false,
					...
					if ($.fn.wl_Chart)
						$.fn.wl_Chart.defaults = {
							...
							tooltipPattern : function(value, legend, label, id) {
								return "value is " + value + " from " + legend
										+ " at " + label + " (" + id + ")";
							},
							// tooltipPattern: "value is %1 from %2 at %3 (%4)",
							// //also possible
							orientation : 'horizontal',
							colors : [ '#b2e7b2', '#f0b7b7', '#b5f0f0',
									'#e8e8b3', '#efb7ef', '#bbb6f0' ],
							flot : {},
					...
					if ($.fn.wl_Fileexplorer)
						$.fn.wl_Fileexplorer.defaults = {
							url : 'elfinder/php/connector.php',
							toolbar : [ [ 'back', 'reload', 'open', 'select',
									'quicklook', 'info', 'rename', 'copy',
									'cut', 'paste', 'rm', 'mkdir', 'mkfile',
									'upload', 'duplicate', 'edit', 'archive',
									'extract', 'resize', 'icons', 'list',
									'help' ] ]
						};
					...
					if ($.fn.wl_Form)
						$.fn.wl_Form.defaults = {
							...
							text : {
								...
								passwordmatch : 'Não é a mesma senha',
								fileinqueue : 'There is at least one file in the queue',
								incomplete : 'Por favor, responta ao formulário corretamente',
								send : 'Enviando...',
					...
					if ($.fn.wl_Password)
						$.fn.wl_Password.defaults = {
							confirm : true,
							showStrength : true,
							words : [ 'too short', 'bad', 'medium', 'good',
									'very good', 'excellent' ],
							minLength : 3,
							text : {
						...
						$.fn.wl_URL.defaults = {
							...
							onChange : function(element, value) {
								if (value != ''
										&& !/^(ftp|http|https):\/\//
												.test(value))
									element.val('http://' + value).trigger(
											'change.wl_Valid');
							}
						};
